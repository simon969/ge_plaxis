...
from .const import (PLX_CMD_NEW, PLX_CMD_CLOSE, PLX_CMD_OPEN, PLX_CMD_RECOVER,
...
class Server(object):
    ...
    def __init__(self, connection, proxy_factory, input_processor, allow_caching=True, error_mode=None):
        """
    ...
    def call_and_handle_command(self, command):
        ...
        """
        return self.call_and_handle_commands(command)[0]
...
def _get_argument(arg_name):
    for arg in sys.argv:
        if arg_name in arg.lower():
            ignore, value = arg.split('=', 1)
            return value

    raise Exception("Couldn't get {} from command line arguments.".format(
        arg_name))
...
def new_server(address=None, port=None, timeout=5.0, request_timeout=None, password=None, error_mode=()):
    ...
    s = Server(conn, pf, ip, error_mode=error_mode)
